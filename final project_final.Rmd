...
Fire 100622 
Traffic 230208
we have 230K traffic incident calls accident in 5 years

These traffic incidents could be a disabled vehicles or road blocks report calls ,  we will fetch only the vehicle accident calls 

The number of calls of accidents is ***129909*** after fetching only accidents and removed duplicates (calls made on same accidents)
...
***Are there any trends during the five years ?*** 
...
Y <- ts(acc_data[,2],start=c(2015),end=c(2020),frequency = 12)
...
```
## Let us check the seasonalities ,,, August is the warmest time of the year that have the highest score of tourists
***can temperature describe the number of of accidents ? are they correlated ?***

```{r , eval=TRUE}
knitr::include_graphics('temp annual.PNG')


```

```{r, eval=TRUE}
knitr::include_graphics('tourism scoring.PNG')
...
#((55814-48737)*100)/48737
#14.5%  


#knitr::include_url("https://f-fbt.rstudio.cloud/7a8dd5a5ca034dc98a2f176b85632e19/file_show?path=%2Fcloud%2Fproject%2F000006.png")
```
### the correclation is 50% 


let us dig deep and check the daily behavior of accidents

***what time of the day accidents are happening ?***  


```{r, eval=TRUE}
dummy <- separate(my_data, col = timeStamp, into = c("Date", "time"), sep = " ") %>% select(time)
dummy$time <-substring(dummy$time,1,2)
dummydata <- dummy %>% count(time)
#dum_data <- read_csv("files/dum_data_date.csv")
#dum_data <- dum_data1 %>% select(-X1)

#clean_tbyhour <- dum_data %>% select(-var_Fire,-var_EMS,-rn,-Type, -timeStamp)
#timeby_day <- clean_tbyhour %>% group_by(hour) %>% summarise(count=n())
r_hour <- highchart() %>% hc_title(text="TimeSeries of vehicle accidents reports spread over a day",style=list(color="red",fontText="Verdana")) %>% hc_xAxis(categories=dummydata$time) %>% hc_add_series(name="count of accidents",data=dummydata$n) %>% hc_add_theme(hc_theme_google())
r_hour
#knitr::include_url("https://f-fbt.rstudio.cloud/7a8dd5a5ca034dc98a2f176b85632e19/file_show?path=%2Fcloud%2Fproject%2Frhour.html")
```

Accidents are mainly happening during rush hour 
the data tells us that the daily time average of accidents are during work time 

***which road has the highest frequency of accidents ?*** 
...
***what about seasons or monthly avaerage for the past 5 years ?*** 
...
acc_data$month=month(acc_data$date)

mnfreq <- acc_data %>% group_by(month) %>% summarise(count=n())
r <- highchart() %>% hc_title(text="TimeSeries of vehicle accidents reports spread over each month ",style=list(color="red",fontText="Verdana")) %>% hc_xAxis(categories=mnfreq$month) %>% hc_add_series(name="count of accidents",data=mnfreq$count) %>% hc_add_theme(hc_theme_google())
...
```{r, eval=TRUE}
knitr::include_graphics('rainy.PNG')
```



***How about we check the correlation between the two plots ***


```{r, eval=TRUE}
#mnfreq

rain_days <- data.frame("month" = 1:12, "rainy" = c(8.6,7.6,8.8,10.2,10.3,8.8,9.2,7.9,7.6,7.4,8.4,8.8))


#df <- as.data.frame(lapply(df, rep, df$ntimes))


k <-  merge(mnfreq,rain_days, by = "month")
lp <- as.data.frame(lapply(k, rep, 5))



log <- log2(k)
ggplot(log, aes(x=count, y=rainy)) + geom_point()  + geom_smooth(method = lm) + ggtitle("cor = 0.70")
cor(k$count, k$rainy)
#[1] 0.700416
```


The coorelation is 70% which is very good 





***let us do a forecast machine learning module ***  
...
## we make a time series for calls 

Y <- ts(fixed_ts[,2],start=c(2015),end=c(2020),frequency = 12)
autoplot(Y) + ggtitle("calls") + ylab("")
...
we will look at the training and test comparison 
```{r, eval=TRUE}
accuracy(fcst, test)
```
